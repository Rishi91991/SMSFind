...
namespace SMS
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.button1.Text = "Host TCP Server";
        ...
        {
            listener = new TcpListener(2400);
            MessageBox.Show("Hosted at port 2400");
            listener.Start();
        ...
        public static void Service()
        {
            while (true)
            {
               ...
               // MessageBox.Show("Connection Accepted at port 2400");
                try
                {
                    Stream s = new NetworkStream(soc);
                    StreamReader sr = new StreamReader(s);
                    string name = sr.ReadLine();
                    string number = name.Substring(0,10);
                    string message = name.Substring(10,name.Length-10);
                   
                    //SEND MESSAGE
                    //MessageBox.Show(message.Length+"");
                    string trimmessage;
                    int temp = message.Length-1;
                    for (int cnt = 0; cnt < temp;)
                    {
                       //string trimmessage=message;
                        if (message.Length <= 70)
                        {
                            //MessageBox.Show(message);
                            //sendmsg("hii","9870509941");
                            sendmsg(message+"", number+"");
                            cnt = cnt + message.Length;
                        }
                        else {
                            trimmessage = message.Substring(0, 69);
                            message = message.Substring(70,message.Length-70);
                            //sendmsg("hii", "9870509941");
                            sendmsg(trimmessage+"",number+"");
                            //MessageBox.Show(trimmessage);
                            cnt = cnt + 70;
                        }
                        ...
                        try
                        {
                            /*   // Send an SMS message
                               ...
                               if (!alert && !unicode)
                               {
                                   ...
                                   pdu = new SmsSubmitPdu(name, "9870509941", "");  // "" indicate SMSC No
                               }
                               else
                               {
                                   ...
                                   pdu = new SmsSubmitPdu(name, "9870509941", "", dcs);
                               }
